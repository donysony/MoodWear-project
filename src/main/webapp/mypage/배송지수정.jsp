...
<%
	...
	//회원의 주소정보 가져오기
	Address addressinfo = new MemberDAO().getAddress(userID, address_num);
	String address_name = addressinfo.getAddress_name();
	String recipient = addressinfo.getAddress_reciever();
	String address1 = addressinfo.getAddress1();
	String address2 = addressinfo.getAddress2();
	int zipcode = addressinfo.getAddress_zipcode();
	String phone = addressinfo.getAddress_phone();
...
<body>
    ...
    <section>
        <article>
            <h2>배송지 수정</h2>
            <hr>
            <form action="배송지수정Action.jsp" method="post" name="shippingFrm">
            <table class="addressinfo_updatetbl">
                    <tr>
                        <th>배송지명</th>
                        <td><input type="text" name="address_name" value="<%=address_name%>"></td>
                    </tr>
                    <tr>
                        <th>받는사람</th>
                        <td><input type="text" name="address_reciever" id="" value="<%=recipient%>"></td>
                    </tr>
                    <tr>
                        <th>연락처</th>
                           <td>
                    ...
                    <tr>
                        <th>주소</th>
                         <td class="address">
                            <input type="text" id="postcode" name="address_zipcode" value="<%=zipcode %>">&ensp;
                            <input type="button" class="zip_code" onClick="findAddr()" value="우편번호"><br>
                            <input type="text" name="address1" class="address_box" id="addr" value="<%=address1 %>"><br>
                            <input type="text" name="address2" class="address_box" id="detailAddr" value="<%=address2 %>" placeholder="상세주소를 넣어주세요."><br>
                            <span id="box"><input type="checkbox" name="" id="default_address"><span>&ensp;기본배송지로 선택</span></span>
                        </td>

                </table>
                <div class="buttondiv">
                    <button type="button" id="update" onclick="register()">수정하기</button> &emsp;
                    <input type="hidden" name="address_num" value="<%=address_num %>"> <!-- 필요할까? -->
